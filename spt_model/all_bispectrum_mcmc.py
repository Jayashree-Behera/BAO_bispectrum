...
def curvefit_fullBk(kk,bkm):
    ...
    bopt, bcov = curve_fit(model_fullBk,kk,bkm,b0,1/r,maxfev=10000000)
    return bopt
...
#     # GLAM filepaths for different z
#     for file in os.listdir(filepaths):
#         if file[-3:] == "npz" and file[:2] == "bk" and "mehdi" not in file and file.endswith(f"{parsed_args.index:02}.npz"):
...
#             output_path = "/global/homes/s/shreeb/BAO_bispectrum/nobao_project/all_mcmc_results/new_alpha/glam_parallel_kmax02/mcmc_" + Path(bpath).stem[3:] + "_p2.h5"
#             mcmc_chain.mcmc_run(output_path = output_path)
    ...
    for file in os.listdir(filepaths):
        if file[-3:] == "dat" and file.endswith(f"{parsed_args.index:04}.dat"):
            ...
            print(h,filepath)
            # _,f,b1,b2,_,S0,S1 = curvefit_fullBk(kg,bg)
            print(curvefit_fullBk(kg,bg))
            # output_path = "/pscratch/sd/s/shreeb/shreeb/BAO_Bispectrum_with_data/nobao_project/all_mcmc_results/new_alpha/glam_parallel_kmax02_params7/mcmc_" + Path(filepath).stem + ".h5"
            output_path = "/global/homes/s/shreeb/BAO_bispectrum/nobao_project/all_mcmc_results/mcmc_cosmoprimo_k2_" + Path(filepath).stem + ".dat"
